...
```
```{r}
fda_drugs <- read_csv('Data_for_app/FDA_df2.csv')
```
```{r}
...
```{r}
food <- read_csv('Data_for_app/food_data_update.csv')
```
...
```{r}
write.csv(choke_food, file = 'Data_for_app/choke_foods.csv', row.names = FALSE)
```
